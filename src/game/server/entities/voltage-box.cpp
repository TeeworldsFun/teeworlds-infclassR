...
#include <game/server/entities/character.cpp>
	...
	: CInfCEntity(pGameContext, EntityId, CenterPos, Owner, BoxProximityRadius)
{
	...
	m_Charges = g_Config.m_InfVoltageBoxCharges;
...
{
	const CEntity *pCharacter = GameServer()->m_apPlayers[ClientID]->GetCharacter();
	if(!pCharacter)
	{
